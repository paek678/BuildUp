...
**절대 규칙: 코드 수정, MCP 도구 사용, 파일 생성 등 실행 행위를 Codex 승인 없이 먼저 하지 않는다.**
최소 1회 교차 검증을 거친 뒤에만 실제 구현을 진행한다.

1. Claude Code가 기획/설계/변경 계획을 작성한다
2. 사용자에게 **Codex에 전달할 검증용 프롬프트**를 함께 출력한다 (코드 수정이나 도구 실행 없이 텍스트만)
3. 사용자가 Codex의 승인/수정/거부 결과를 가져온다
4. Claude Code가 Codex 피드백을 반영하여 최종 판단 후 구현을 진행한다
